...
const CropShowcase = () => {
  ...
  const [selectedCropToSend, setSelectedCropToSend] = useState<Crop | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const plantedCrops = await apiRequest(
          `/planted-crops/farm/${farmId}`,
          "GET"
        );
  ...
  const handleUpdateCrop = async (crop: Crop) => {
    try {
      ...
      const updatedCrop = await apiRequest(
        `/planted-crops/${id}`,
        "PUT",
        cropToSend
      );
